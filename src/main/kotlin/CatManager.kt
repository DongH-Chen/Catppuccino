...
// mask
private const val FRAME_NUM_MASK = 0xff
private const val BUBBLE_FRAME_MASK = 0xff00
private const val BUBBLE_STEPS_MASK = 0xff0000
private const val ANIMATION_STEPS_MASK = 0xff000000.toInt()

// offset
private const val FRAME_NUM_SHIFT = 0
private const val BUBBLE_FRAME_SHIFT = 8
private const val BUBBLE_STEPS_SHIFT = 16
private const val ANIMATION_STEPS_SHIFT = 24
...
object CatManager {
    ...
    val wanderLoc = Point(0, 0)
    var bubbleState = BubbleState.NONE

    // 0-7: frameNum (0-255)
    // 8-15: bubbleFrame (0-255)
    // 16-23: bubbleSteps (0-255)
    // 24-31: animationSteps (0-255)
    private var packedValues = 0
    var frameNum: Int
        get() = (packedValues and FRAME_NUM_MASK) shr FRAME_NUM_SHIFT
        set(value) {
            packedValues = (packedValues and FRAME_NUM_MASK.inv()) or (value shl FRAME_NUM_SHIFT)
        }
    var bubbleFrame: Int
        get() = (packedValues and BUBBLE_FRAME_MASK) shr BUBBLE_FRAME_SHIFT
        set(value) {
            packedValues = (packedValues and BUBBLE_FRAME_MASK.inv()) or (value shl BUBBLE_FRAME_SHIFT)
        }
    var bubbleSteps: Int
        get() = (packedValues and BUBBLE_STEPS_MASK) shr BUBBLE_STEPS_SHIFT
        set(value) {
            packedValues = (packedValues and BUBBLE_STEPS_MASK.inv()) or (value shl BUBBLE_STEPS_SHIFT)
        }
    var animationSteps: Int
        get() = (packedValues and ANIMATION_STEPS_MASK) shr ANIMATION_STEPS_SHIFT
        set(value) {
            packedValues = (packedValues and ANIMATION_STEPS_MASK.inv()) or (value shl ANIMATION_STEPS_SHIFT)
        }

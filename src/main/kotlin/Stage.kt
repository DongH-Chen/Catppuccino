...
class Stage : JPanel() {
    ...
    override fun paintComponent(g: Graphics?) {
        val img = CatManager.currFrames.getOrNull(CatManager.frameNum) ?: return
        val flippedImage = if (needsFlipping(CatManager.currentAction, CatManager.layingDir)) flipImg(img) else img
        g?.drawImage(flippedImage, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, null) ?: return
        if (CatManager.bubbleState != BubbleState.NONE) {
            CatManager.currBubbleFrames.getOrNull(CatManager.bubbleFrame)?.let { bubbleImg ->
                val position = calculateBubblePosition(CatManager.currentAction, CatManager.layingDir)

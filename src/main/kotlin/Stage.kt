...
class Stage : JPanel() {
    ...
    private fun flipOrNot(behave: Behave, direction: Direction) =
    ...
    private fun calculateBubblePosition(action: Behave, direction: Direction): Point {
        return when (action) {
            Behave.SLEEP, Behave.LAYING, Behave.LEFT, Behave.RIGHT ->
                Point(if (direction == Direction.LEFT) 0 else 60, 40)
    ...
    override fun paintComponent(g: Graphics?) {
        ...
        val flippedImage = if (flipOrNot(currentAction, layingDir)) flipImg(img) else img
